...
void LevelB::initialise()
{
    ...
    background_texture_id = Utility::load_texture("assets/space.png");
    ...
    GLuint enemy_bullet_tex_id = Utility::load_texture("assets/enemy_bullet.png");
    ...
    for (int i=0;i<ENEMY_COUNT;++i) {
        if (i<5) {
            ...
            m_game_state.enemies[i]->set_cooldown(1.2f);
        }
        else if (i>12) {
            if (i==15|i==18|i==21) m_game_state.enemies[i] = new Entity(large_enemy_texture_id, 0.8f, 0.8f, ENEMY);
            else m_game_state.enemies[i] = new Entity(little_enemy_texture_id, 0.8f, 0.8f, ENEMY);
            m_game_state.enemies[i]->set_cooldown(1.2f);
            
        }
        else {
            m_game_state.enemies[i] = new Entity(little_enemy_texture_id, 1.0f, 1.0f, ENEMY);
            m_game_state.enemies[i]->set_cooldown(0.5f);
        }
        ...
        // enemies' bullet
        for (int j=0; j<16; ++j) {
            m_game_state.enemies[i]->bullets[j] = new Entity(enemy_bullet_tex_id, 1, 1, 1, BULLET);
            m_game_state.enemies[i]->bullets[j]->set_width(0.3f);
            m_game_state.enemies[i]->bullets[j]->set_height(0.5f);
            m_game_state.enemies[i]->bullets[j]->set_position(glm::vec3(7.0f, 0.0f, 0.0f)); // out of the screen waiting to be used
            if (i<5) {
                m_game_state.enemies[i]->bullets[j]->set_velocity(glm::vec3(0.0f, 0.5f, 0.0f));
            }
            else {
                m_game_state.enemies[i]->bullets[j]->set_velocity(glm::vec3(0.0f, 1.5f, 0.0f));
            }
        }
    }
    // Wave 1
    float wave1_loc = 7.0f;
    m_game_state.enemies[0]->set_position(glm::vec3(-4.0f, 3.0f+wave1_loc, 0.0f));
    m_game_state.enemies[1]->set_position(glm::vec3(-2.0f, 2.5f+wave1_loc, 0.0f));
    m_game_state.enemies[2]->set_position(glm::vec3(0.0f, 2.0f+wave1_loc, 0.0f));
    m_game_state.enemies[3]->set_position(glm::vec3(2.0f, 1.5f+wave1_loc, 0.0f));
    m_game_state.enemies[4]->set_position(glm::vec3(4.0f, 1.0f+wave1_loc, 0.0f));
    // Wave 2
    float wave2_loc = 14.0f;
    m_game_state.enemies[5]->set_position(glm::vec3(-4.0f, 0.6f+wave2_loc, 0.0f));
    m_game_state.enemies[6]->set_position(glm::vec3(-3.2f, 0.6f+wave2_loc, 0.0f));
    m_game_state.enemies[7]->set_position(glm::vec3(-2.2f, 1.4f+wave2_loc, 0.0f));
    m_game_state.enemies[8]->set_position(glm::vec3(-1.4f, 1.4f+wave2_loc, 0.0f));
    m_game_state.enemies[9]->set_position(glm::vec3(-0.4f, 2.2f+wave2_loc, 0.0f));
    m_game_state.enemies[10]->set_position(glm::vec3(0.4f, 2.2f+wave2_loc, 0.0f));
    m_game_state.enemies[11]->set_position(glm::vec3(1.4f, 3.0f+wave2_loc, 0.0f));
    m_game_state.enemies[12]->set_position(glm::vec3(2.2f, 3.0f+wave2_loc, 0.0f));
    // Wave 3
    float wave3_loc = 21.0f;
    m_game_state.enemies[13]->set_position(glm::vec3(-4.0f, 3.0f+wave3_loc, 0.0f));
    m_game_state.enemies[14]->set_position(glm::vec3(-3.2f, 3.0f+wave3_loc, 0.0f));
    m_game_state.enemies[15]->set_position(glm::vec3(-3.6f, 2.0f+wave3_loc, 0.0f)); // big head
    m_game_state.enemies[16]->set_position(glm::vec3(-0.4f, 2.0f+wave3_loc, 0.0f));
    m_game_state.enemies[17]->set_position(glm::vec3(0.4f, 2.0f+wave3_loc, 0.0f));
    m_game_state.enemies[18]->set_position(glm::vec3(0.0f, 1.0f+wave3_loc, 0.0f)); // big head
    m_game_state.enemies[19]->set_position(glm::vec3(4.0f, 3.0f+wave3_loc, 0.0f));
    m_game_state.enemies[20]->set_position(glm::vec3(3.2f, 3.0f+wave3_loc, 0.0f));
    m_game_state.enemies[21]->set_position(glm::vec3(3.6f, 2.0f+wave3_loc, 0.0f)); // big head
...
{
    if (m_game_state.player->get_position().y>55.0f) m_game_state.next_scene_id = 3;
...
void LevelB::spawnExplosion(const glm::vec3& pos) {
