...
void ShaderProgram::load(const char *vertex_shader_file, const char *fragment_shader_file) {
    ...
    m_spotlight_left_uniform = glGetUniformLocation(m_program_id, "spotlightLeft"); // load the address for both lights
    m_spotlight_right_uniform = glGetUniformLocation(m_program_id, "spotlightRight");
    
    set_colour(1.0f, 1.0f, 1.0f, 1.0f);
    
}

// get plane's positions from main and pass it to the shader by addresses
void ShaderProgram::set_spotlight_positions(const glm::vec2 &left, const glm::vec2 &right) {
    glUniform2f(m_spotlight_left_uniform, left.x, left.y);
    glUniform2f(m_spotlight_right_uniform, right.x, right.y);
}
